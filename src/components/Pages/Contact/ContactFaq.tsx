      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4 sm:space-y-5">
          {faqs.map((faq, index) => (
            <motion.div
              key={index}
              className="bg-white rounded-xl sm:rounded-2xl overflow-hidden border border-gray-200 hover:border-purple-300 transition-all duration-300 shadow-sm hover:shadow-md"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{
            ...
            >
              <button
                ...
                className="w-full px-5 xs:px-6 sm:px-8 py-4 xs:py-5 sm:py-6 text-left flex items-center justify-between gap-4 hover:bg-purple-50 transition-colors"
              >
                <h3 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-900 pr-2">
                  {faq.question}
                </h3>
                <motion.div
